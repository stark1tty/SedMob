...
Each recipe is a Python class in `p4a-recipes/<package>/__init__.py` that extends `PythonRecipe`. Both download source from GitHub. Flask uses the default PythonRecipe install, while Werkzeug overrides `install_python_package` to use `pip install` because Werkzeug 3.x uses `pyproject.toml` (flit) instead of `setup.py`. If you need to pin or override another dependency for the APK build, add a new recipe following the same pattern.
